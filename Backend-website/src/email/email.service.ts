...
export class EmailService implements OnModuleInit {
  ...
  private provider: 'resend' | 'smtp' = 'smtp';

  constructor() {
    // Prefer Resend on hosted environments to avoid SMTP egress issues
    ...
    if (resendKey) {
      ...
      const secure = port === 465; // implicit TLS for 465, STARTTLS otherwise
...
<body>
  ...
  private async sendEmail(
    ...
    if (this.provider === 'resend' && this.resend) {
      try {
        const from = process.env.RESEND_FROM || 'UJGSM <noreply@uorapublications.com>';
    ...
    if (!this.transporter) {
      this.logger.error('No SMTP transporter configured');
      return;
    }
    ...
    const shouldRetry = (err: any) => {
      const transient = ['ETIMEDOUT', 'ECONNECTION', 'EAI_AGAIN', 'ECONNRESET'];
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        this.logger.log(`Email sent via SMTP to ${to} (attempt ${attempt})`);
        return;
      } catch (error: any) {
        const last = attempt === maxAttempts;
        this.logger.error(`SMTP email failed (attempt ${attempt}/${maxAttempts}) to ${to}: ${error?.message}`);
        if (!shouldRetry(error) || last) return;
        const delay = 500 * attempt ** 2; // 500ms, 2000ms
...
export class EmailService {
  ...
  constructor() {
    // If RESEND_API_KEY is set, prefer Resend on Railway to avoid SMTP egress blocks
    const resendKey = process.env.RESEND_API_KEY;
    if (resendKey) {
      this.resend = new Resend(resendKey);
      this.provider = 'resend';
